...
class WeatherApp(QWidget):
    ...
    def get_weather(self):
        ...
        api_key = "bf212a9db8268051aad01083a0b8a042"
        city = self.city_input.text()
        url = f"https://api.openweathermap.org/data/2.5/weather?q={city}&appid={api_key}"

        try:
            response = requests.get(url)
            response.raise_for_status() #this will raise an exception if their is an http error
            data = response.json()
...
if __name__ == '__main__':
    ...
    weather_app.show()
    sys.exit(app.exec_()) #this is a method that handles event within an app (such as closing the window)